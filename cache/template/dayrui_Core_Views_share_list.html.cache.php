...
<script type="text/javascript">
    ...
    function dr_module_delete() {
        var url = '<?php echo dr_url(APP_DIR.'/home/del'); ?>';
        var width = '50%';
        var height = '60%';
        if (is_mobile_cms == 1) {
            width = height = '90%';
    ...
    <div class="table-scrollable">
        <table class="table table-striped table-bordered table-hover table-checkable dataTable">
            ...
            <tr class="heading">
                ...
                <?php } else { ?>
                <th width="90" class="<?php echo dr_sorting('id'); ?>" name="id">Id</th>
            ...
            <tr class="odd gradeX" id="dr_row_<?php echo $t['id']; ?>">
                ...
                <td style="overflow:visible">
                    ...
                    <?php }  if (is_array($clink)) { $count=count($clink);foreach ($clink as $a) {  if (!$a['uri'] || ($a['uri'] && \Phpcmf\Service::C()->_is_admin_auth($a['uri']))) { ?>
                    <label><a class="btn green btn-xs" href="<?php echo str_replace(array('{mid}', '{id}', '{cid}'), array(APP_DIR, $t['id'], $t['id']), $a['url']); ?>"><i class="<?php echo $a['icon']; ?>"></i> <?php echo dr_lang($a['name']);  if ($a['field']) { ?> (<?php echo intval($t[$a['field']]); ?>)<?php } ?> </a></label>
                    <?php }  } }  if (is_array($module['form'])) { $count=count($module['form']);foreach ($module['form'] as $a) {  if (\Phpcmf\Service::C()->_is_admin_auth(APP_DIR.'/'.$a['table'].'/index')) { ?>
                    ...
                    <label><a href="<?php echo dr_url(APP_DIR.'/comment/index', ['cid'=>$t['id']]); ?>" class="btn btn-xs dark"> <i class="fa fa-comments"></i> <?php echo dr_lang('%s(%s)', '评论', intval($t['comments'])); ?> </a></label>
                    <?php } ?>
                </td>
            </tr>
            <?php } } ?>
            ...
            <label>
                <div class="btn-group dropup">
                    ...
                    <ul class="dropdown-menu">
                        <li>
                            <a href="javascript:;" onclick="dr_module_send('<?php echo dr_lang("推荐位"); ?>', '<?php echo dr_url(APP_DIR.'/home/tui_edit'); ?>&page=0')"><i class="fa fa-flag"></i> <?php echo dr_lang('推送到推荐位'); ?> </a>
                        </li>
                        <?php if ($module[setting][sync_category]) { ?>
                        <li>
                            <a href="javascript:;" onclick="dr_module_send('<?php echo dr_lang("发布"); ?>', '<?php echo dr_url(APP_DIR.'/home/tui_edit'); ?>&page=1')"><i class="fa fa-refresh"></i> <?php echo dr_lang('发布到其他栏目'); ?> </a>
                        </li>
                        <?php }  if ($weibo['use']) { ?>
                        <li>
                            <a href="javascript:;" onclick="dr_module_send_ajax('<?php echo dr_url(APP_DIR.'/home/tui_edit'); ?>&page=3')"><i class="fa fa-weibo"></i> <?php echo dr_lang('分享到官方微博'); ?> </a>
                        </li>
                        <?php } ?>
                        <li>
                            <a href="javascript:;" onclick="dr_module_send('<?php echo dr_lang("微信"); ?>', '<?php echo dr_url(APP_DIR.'/home/tui_edit'); ?>&page=2')"><i class="fa fa-weixin"></i>  <?php echo dr_lang('发送到微信公众号'); ?> </a>
                        </li>
                        <li>
                            <a href="javascript:;" onclick="dr_module_send_ajax('<?php echo dr_url(APP_DIR.'/home/tui_edit'); ?>&page=4')"><i class="fa fa-clock-o"></i>  <?php echo dr_lang('更新时间'); ?> </a>
                        </li>
                        <?php if (is_array($cbottom)) { $count=count($cbottom);foreach ($cbottom as $a) {  if (!$a['uri'] || ($a['uri'] && \Phpcmf\Service::C()->_is_admin_auth($a['uri']))) { ?>
                        <li>
