...
function maketxt($bookid)
{
	global $dsql,$cfg_webname,$cfg_basehost,$co_addtxtpsize;
...
if(is_array($row))
{
	if(isset($_GET['cm']))
	{
		...
		if($isdel=="")
		{
			...
			$sss.="共采集了 $cm 次,【本条规则ID：".$nid."】【".$notename2."】<br />";
			...
			if($channelid>10) $co->GetSourceUrl(1,1,50);
			...
			for($n=0;$n<$con;$n++)
			{
					...
					if(is_array($row))
					{
						...
						if($channelid<10)
						{
							...
							if(!is_array($tcount))
							{
								...
								$sss.= " 【<font color='red'>小说ID[".$c_typeid."]不存在，已删除规则</font>】<br>";
								break;
							}
						...
						if($channelid<10)
						{
							if(in_array($cc_title,$paichuarray))
							{
								...
								$sss.= " 【<font color='red'>$a_title-不符合要求-2</font>】<br>";
								continue;
							}
							...
							{
								$dsql->ExecuteNoneQuery("update `#@__co_htmls` set isexport=1,isdown=1 where aid='$aids' ");
								$sss.= " 【<font color='red'>重复章节：$a_title ，跳过采集</font>】<br>";
								continue;
							}
						}
							...
							if($suc=="no")
							{
								if($coretag=="re" && $coretimes>=$co_retime)
								{
									if($co_eorctr!='2' && $cotype!='2')
									{
										...
										{
											$dsql->ExecuteNoneQuery("update `#@__co_note` set remark= CONCAT(remark,'目标站访问错误，ID：','$aids'),typeid='0' where `nid`=$nid ");
										}
										$dsql->ExecuteNoneQuery("update `#@__co_htmls` set isdown=0,result='' where aid='$aids' ");
										$sss.= "【<font color='red'>网址内容".$row['url']."连续".$co_retime."次采集失败，取消本规则的采集</font>】<br>";
									}
									else
									{
										$dsql->ExecuteNoneQuery("update `#@__co_htmls` set isdown=1,isexport=1 where aid='$aids' ");
										$sss.= "【<font color='red'>网址内容".$row['url']."连续".$co_retime."次采集失败，跳过该网址继续采集</font>】<br>";
									}
								}
								else
								{
									...
									$sss.= "【<font color='red'>网址内容".$row['url']."连续".$co_retime."次采集失败，等待重试</font>】<br>";	
								}
								break;
							}
						}
						...
						{
							$dsql->ExecuteNoneQuery("update `#@__co_htmls` set isexport=1,isdown=1 where aid='$aids' ");
							$sss.= "【<font color='red'>网址内容".$row['url']."下载失败</font>】<br>";
						}
					}
					else if($renid1=="1")
					{
						...
						if(is_numeric($overtypeid))
						{
							...
							if($overrow['dd']==0)
							{
								$sss.= "【<font color='red'>采集不到章节，请检查目标站是否能正常访问！</font>】<br>";
								break;
							}
							else
							{
								...
								if($overrow['dd']>0)
								{
									...
									$sss.= "【<font color='red'>符合完本条件，已删除本条规则！</font>】<br>";
									break;
								}
								else
								{
									...
									for($m=0;$m<count($endkey);$m++)
									{
										...
										if($endrow['dd']>0)
										{
											...
											$sss.= "【<font color='red'>符合完本条件，已删除本条规则！</font>】";
											break 2;
										}
									}
								}
				...
				while($row = $dsql->GetObject())
				{
					...
					if($channelid>10)
					{
						...
						if($title=="" || $writer=="" || $bookurl=="" || ($channelid==99 && $source==""))
						{
							...
							else if($n==$nn)
							{
								if($coretag=="re" && $coretimes==$co_retime)
								{
									...
									$sss.= "【<font color='red'>小说“".$title."”连续".$co_retime."次都采集不到内容，放弃采集</font>】<br>";
									$eorid=0;
									continue;
								}
								else
								{
									...
									$sss.= "【<font color='red'>小说“".$title."”连续".$co_retime."次都采集不到内容，等待重试</font>】<br>";	
									break 2;
								}
							}
							else
							{
								...
								$sss.= "【<font color='red'>小说“".$title."”内容是空的，重新采集</font>】<br>";
								break;
							}
						}
						if($nn>0)
						...
						{
							$dsql->ExecuteNoneQuery("update `#@__co_htmls` set isexport=1 where aid='$exid' ");
							$sss.= " 【<font color='red'>小说已存在</font>】<br>";
							continue;
						}
					...
					else
					{
							...
							else if($n==$co_retime)
							{
								if($coretag=="re" && $coretimes==$co_retime)
								{
									if($co_nullctr!='2' && $cotype!='2')
									{
										...
										$dsql->ExecuteNoneQuery("update `#@__co_htmls` set isdown=0,result='' where aid='$exid' ");
										$sss.= "【<font color='red'>章节“".$title."”连续".$co_retime."次都采集不到内容，取消本规则的采集</font>】<br>";
									}
									else
									{
										$eorid=0;
								...
								else
								{
									$dsql->ExecuteNoneQuery("update `#@__co_htmls` set isdown=0,result='' where aid='$exid' ");
									$dsql->ExecuteNoneQuery("update `#@__co_note` set remark= CONCAT('re:','$coretime'),cotime=cotime+3600 where `nid`=$nid ");
									$sss.= "【<font color='red'>章节“".$title."”连续".$co_retime."次都采集不到内容，等待重试</font>】<br>";	
								}
								$treid2='b';
								break 2;
							}
						...
						if($only)
						{
							...
							if(is_array($testrow))
							{
								...
								{
									$dsql->ExecuteNoneQuery("update `#@__co_htmls` set isexport=1 where aid='$exid' ");
									$sss.= " 【<font color='red'>重复章节：$title ，跳过采集</font>】<br>";
									continue;
								}
								else
								{
								   ...
								   VALUES ('$taid','$typeid','$body','','',''); ");
									...
									$dsql->ExecuteNoneQuery("update `#@__co_htmls` set isexport=1 where aid='$exid' ");
									$sss.= " 【<font color='green'>已更新章节：$title</font>】<br>";
									continue;
								}
							}
						}
								...
								else
								{
									...
									if($row)
									{
										...
										if($pretid!=$typeid && $pretid!='c')
										{
											...
											$sss.= '<br/>开始更新《'.$ptypename.'》';
											flushtemp( $tid );
											if($treid2!=$treid && $treid2!='b')
											{
												$tid=$treid2;
												flushtemp( $tid );
												$sss.= '，更新分类：'.$tretypename2;
											}
											$sss.= '-成功';
										}
										$treid2=$row['reid'];
			...
			if($nidcon=='0' and $channelid==1 and $booksum2>1 and $cotype>0)
			{
				...
				for($a=0;$a<count($tidarr[1]);$a++)
				{
					...
					if(is_array($endrow))
					{
						$sss.="【<font color='red'>".$endrow['typename']."[".$endrow['id']."]->".$endrow['lastt']."</font>】->";
		...
		{
			$sss.="错误或者无用的规则，已删除！<br />";
		}
	}
}
else
{
	$sss.="没有符合条件的采集规则，请检查您的设置！<br />";
}
$dsql->Close();
$runtime->stop();
if(isset($_GET['cm']))
	$sss.="页面执行时间: ".$runtime->spent()." 毫秒（".date("Y-m-d H:i:s",time()+1)."）<br />";
else
	$co_time=3;
$sss.= '<script type="text/JavaScript">parent.document.getElementById("lastmove").value=Math.round(new Date().getTime()/1000);</script>';
$sss.= '<script type="text/JavaScript">parent.document.getElementById("cms").value='.$cm.';</script>';
echo "<p style='font-size:12px'>".$sss."</p>";
$co_time=(intval($co_time)>=0) ? intval($co_time):1;
ShowMsg("采集正在进行中......<br />关闭本页面将停止采集.","http://".$rhost."/download/caiji.php?".$nidurl."cm=$cm",'',$co_time*1000);
?>