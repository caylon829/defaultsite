...
class DedeCollection
{
	...
	function GetHtmlArea_regex($sptag, &$areaRule, &$html)
    {
        //用正则表达式的模式匹配
        $areaRule = str_replace("/", "\\/", $areaRule);
		$areaRules = explode($sptag, $areaRule);
    ...
    function DownOnePage($dourl)
    {
		...
		else $html = file_get_contents($dourl);
    ...
    function DownMedia($dourl, $mtype='img', $islitpic=FALSE)
    {
        ...
        if($tofile=='' || !file_exists($GLOBALS['cfg_basedir'].$filename))
        {
            ...
            //防盗链模式
            if($this->noteInfos['isref']!='no' && $this->noteInfos['refurl']!='')
            {
               /*  if($this->noteInfos['exptime']=='')
				...
				$imgFile = file_get_contents($dourl);
				file_put_contents($GLOBALS['cfg_basedir'].$filename, $imgFile);
            }
        ...
        if($mtype=='img' && !$islitpic && $this->breImage=='')
        {
            ...
            if(!preg_match("#^http:\/\/#", $this->breImage) && file_exists($GLOBALS['cfg_basedir'].$filename))
            {
                ...
                if(@copy($GLOBALS['cfg_basedir'].$filename, $GLOBALS['cfg_basedir'].$nfilename))
                {
					...
					if(@unlink($GLOBALS['cfg_basedir'].$filename))
						rename($GLOBALS['cfg_basedir'].$nfilename, $GLOBALS['cfg_basedir'].$filename);
                }
            }
        }
    ...
    function GetPageFields($dourl, $needDown, $litpic='')
    {
        ...
        foreach($this->artNotes as $k=>$sarr)
        {
            ...
            else
            {
                ...
                {
                    if($k=='body')
						$v = $this->GetHtmlArea_regex('[内容]',$sarr['match'],$this->tmpHtml);
					else
                ...
                if(isset($sarr['trim']) && $v!='')
                {
                    foreach($sarr['trim'] as $nv)
                    {
						...
						if($nvs!="<a([^>]*)>" && $nvs!="<A([^>]*)>" && $nvs!="</a>" && $nvs!="</A>")
						{
							$v = preg_replace("#".$nvs."#isU", $nv[1], $v);
						}
                    }
                }
				$v = preg_replace("#<a([^>]*)>#isU", "", $v);
				$v =str_replace("</a>", "", $v);
				$v =str_replace("</A>", "", $v);
    ...
    function GetSourceUrl($islisten=0, $glstart=0, $pagesize=10)
    {
        ...
        else
        {
            ...
            if($unum>0)
            {
				...
				$nn=0;
				$perconum=$this->noteInfos['exptime'];
                foreach($tmplink as $vs)
                {
					$nn++;
					if($nchannelid>10 && $perconum!=10 && $unum-$nn>=$perconum)
					{
						continue;
					}
                    $v = $vs[0];
                    ...
                    {
                        if($nchannelid>10)
							$lrow = $this->dsql->GetOne("SELECT * FROM `#@__co_urls` a left join `#@__co_note` b on(b.nid=a.nid) WHERE b.channelid=$nchannelid AND (a.hash='".md5(str_replace("23wx","23us",$v['link']))."' or a.hash='".md5($v['link'])."')");
						else
							$lrow = $this->dsql->GetOne("SELECT * FROM `#@__co_urls` WHERE hash='".md5(str_replace("23wx","23us",$v['link']))."' or hash='".md5($v['link'])."' ");
                        if(is_array($lrow))
                        {
                            continue;
                        }
