...
function pr_str_replace($str)
{
	...
	while ($row = $dsql->GetArray())
	{
	  ...
	  {
		  $str = str_replace(trim($row['find']), "@@@@", $str); 
		  $str = str_replace(trim($row['replace']), $row['find'], $str); 
		  $str = str_replace("@@@@", trim($row['replace']), $str); 
	  } */
		$karr[] = trim($row['find']);
		$kaarr[] = trim($row['replace']);
	  /* else if($row['interconvert'] == 0)
	  {
		$str = str_replace(trim($row['find']), trim($row['replace']), $str); 
	  } */
	  $n++;
	}
	if($n>0)
	{
		$strstrarray=array_combine($karr,$kaarr);
		$str=strtr($str,$strstrarray);
	}
	return $str;
}
//add by qisip兼容PHP5.4以上版本htmlout
function htmlout($str) 
{
    return htmlspecialchars($str,ENT_COMPAT,'ISO-8859-1');
}
function filter_GBK_char($src)
{
	$src = str_replace(" ","",$src);
	$src = strval($src);
	$dest = '';
