...
class TypeUnit
{
    ...
    function LogicListAllSunType($id, $step)
    {
        global $cfg_admin_channel, $admin_catalogs;
        $fid = $id;
        $this->dsql->SetQuery("SELECT a.id,reid,a.typedir,a.typename,a.ispart,a.sortrank,a.ishidden,a.overdate,a.lastupdate,a.copynid,b.notename,b.typeid as cotype FROM `#@__arctype` a left join #@__co_note b on(b.nid=a.copynid) WHERE a.reid='".$id."' order by sortrank,id desc");
                ...
                if($ispart==0)
                {
					...
					if(time()-$lastupdate>172800)
					{
						if($notename!="")
						{
							...
							$lastupdate1="<a href=\"co_url.php?nid=".$copynid."\" target=\"_blank\" title=\"".$cotype.",点击打开采集内容列表\"><font color='red'>".date("Y-m-d H:i:s",$lastupdate)."</font></a>[<a href=\"co_do.php?dopost=setcgn&nid=".$copynid."&typename=".$typeName."\" target=\"_blank\" title=\"从其他站采集该小说\">换站采集</a>]";
						}
						else
						{
							$lastupdate1="<font color='red'>".date("Y-m-d",$lastupdate)."</font>[<a href=\"co_do.php?dopost=copy&bookid=".$id."&typename=".$typeName."\" target=\"_blank\" title=\"该小说的采集规则不存在或者被删除，点击添加\">添加采集规则</a>]";
						}
					}
					else $lastupdate1="<a href=\"co_url.php?nid=".$copynid."\" target=\"_blank\" title=\"正在采集中,点击打开采集内容列表\">".date("Y-m-d H:i:s",$lastupdate)."</a>[<a href=\"co_do.php?dopost=setcgn&nid=".$copynid."&typename=".$typeName."\" target=\"_blank\" title=\"从其他站采集该小说\">换站采集</a>]";
					if($overdate!="0")
					{
						if($copynid=="99999999" || $copynid=="" || $notename=="")
							$sumact = "<font color='blue'>【完本】</font>最后更新:".date("Y-m-d",$lastupdate)."[<font color='blue'>已采集完毕</font>]";
						else
							$sumact = "<font color='blue'>【完本】</font>最后更新:".$lastupdate1;
					}
					else
					{
						$sumact = "【连载】最后更新:".$lastupdate1;
					}
					echo "<tr height='24' oncontextmenu=\"CommonMenu(event,this,$id,'".urlencode($typeName)."')\">\r\n";
                    echo "<td class='nbline'>";
                    echo "<table width='98%' border='0' cellspacing='0' cellpadding='0'>";
                    echo "<tr onMouseMove=\"javascript:this.bgColor='#FAFCE0';\" onMouseOut=\"javascript:this.bgColor='#FFFFFF';\"><td width='240px'>";
                    echo "<input class='np' type='checkbox' name='tids[]' value='{$id}'>$step <img style='cursor:pointer' id='img".$id."' onClick=\"LoadSuns('suns".$id."',$id);\" src='images/dedeexplode.gif' width='11' height='11'> <a href='catalog_do.php?cid=".$id."&dopost=listArchives' target=\"_blank\">{$nss}".$typeName."[ID:".$id."]</a>  <a onclick=\"AlertMsg('快捷编辑窗口','$id');\" href=\"javascript:;\"><img src='images/write2.gif'/></a>";
                    echo "</td><td>".$sumact."</td><td align='right'>";
                    echo "<a href='{$GLOBALS['cfg_phpurl']}/list.php?tid={$id}' target='_blank'>预览</a>";
                    echo "|<a href='catalog_do.php?cid={$id}&dopost=listArchives'>内容</a>";
                    echo "|<a href='catalog_add.php?id={$id}'>增加子类</a>";
                    echo "|<a href='catalog_edit.php?id={$id}'>更改</a>";
