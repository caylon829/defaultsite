...
class DataListCP
{
    ...
    function GetPageList($atts,$refObj='',$fields=array())
    {
        ...
        if(preg_match("#form#i", $atts['listitem']))
        {
            ...
            {
                $plist.="<input type='tel' name='pageno' style='padding:0px;width:30px;height:24px;font-size:13px' />\r\n";
                $plist.="<input type='submit' name='plistgo' value='GO' style='padding:0px;width:36px;height:28px;font-size:13px' />\r\n";
            }
            $plist .= "</form>\n";
        }
        if(preg_match("#select#i", $atts['listitem']))
        {
            $plist .='<select name="pagelist" onchange="javascript:location.href=this.value;"> ';
            for($aa=1;$aa<=$totalpage;$aa++)
            {
                $plist.= $aa==$this->pageNO ? '<option value="'.$purl.'pageno='.$aa.'" selected>'.$aa.'</option>':'<option value="'.$purl.'pageno='.$aa.'">'.$aa.'</option>';
            }
            $plist .= "</select>\n";
        }
        $plist .= "</div>\n";
