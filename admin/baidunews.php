...
if(empty($do))
{
    ...
    while($row = $dsql->GetArray())
    {
        ...
        $link = htmlout($link);
        $description = htmlout(strip_tags($row['description']));
        $text = htmlout(strip_tags($row['body']));
        ...
        //$headlineimg = '';
        $keywords = htmlout($row['keywords']);
        $category = htmlout($row['typename']);
        $author = htmlout($row['writer']);
        $source = htmlout($row['source']);
        $pubdate = htmlout(gmdate('Y-m-d H:i',$row['pubdate'] + $cfg_cli_time * 3600));
