...
else if($dopost=='hebingSave')
{
	...
	$Row2 = $dsql->GetOne("SELECT a.title,a.typeid,b.body FROM `#@__archives` a left join `#@__addonarticle` b on(a.id=b.aid) WHERE a.id='$toaid'");
	if($hbtype==1)
	{
	...
	if($dsql->ExecuteNoneQuery(" UPDATE `#@__addonarticle` a,`#@__archives` b SET b.title='$hbtitle',a.body='$hbbody' WHERE a.aid='$toaid' and b.id='$toaid' "))
	{
		...
		//生成txt文件并更新封面和分类
		$murl=$cfg_basehost."/download/maketxt.php?bookid=".$Row2['typeid'];
		$file = fopen($murl,"r");
		fclose($file);
