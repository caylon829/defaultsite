...
{
    $query = " SELECT COUNT(id) AS dd FROM `#@__arctype` where reid not in(0,45)";
    $row1 = $dsql->GetOne($query);
    $query = " SELECT COUNT(id) AS dd FROM `#@__arctype` where reid not in(0,45) and overdate<>'0'";
    $row2 = $dsql->GetOne($query);
    $query = "SELECT COUNT(id) AS dd FROM `#@__arctiny`";
    $row3 = $dsql->GetOne($query);
    
?>
    <table width="100%" class="dboxtable">
    <tr>
        <td width='50%' class='nline'  style="text-align:left"> 小说总数： </td>
        <td class='nline' style="text-align:left"> <?php echo $row1['dd']; ?> (已完本：<?php echo $row2['dd']; ?>)</td>
    </tr>
    <tr>
        <td class='nline' style="text-align:left"> 章节总数： </td>
        <td class='nline' style="text-align:left"> <?php echo $row3['dd']; ?> </td>
    </tr>
    </table>
<?php
exit();
