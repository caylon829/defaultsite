...
*/
	...
	{
        global $cfg_basehost;
		header("Location:http://www.010xr.com/Credits?domain=".str_replace('http://','',$cfg_basehost));		
	}
...
*/
    ...
    {
        $s = $this->downfile("http://www.010xr.com/?m=Caijixia&a=testhttp");
        if($s=='success')
        {
            ShowMsg('您的空间支持采集',-1);
        }else
...
*/
     ...
     function ac_update()
     {
        $str = $this->downfile('http://www.010xr.com/Update/?type=2&plugin=kwrobot&ver='.$this->ver);
...
*/
     ...
     function ac_updatenow()
     {
         ...
         $ver = $this->gv('ver');
         $file = $this->downfile('http://www.010xr.com/uploads/onlineupdate/'.$ver.'_'.$this->gv('soft_lang').'.xml');
...
*/
	 ...
	 function ac_suggest()
	 {
		...
		$url = 'http://www.010xr.com/Suggest?';
	...
	function ac_mykey(){
		header("Location: http://www.010xr.com/?m=caijixia&a=mykey");
	}
